...
class all_parts:
    """ all_parts can be used to join brandnew_structures at ankerpoints"""
    ...
    def get_part_table(self):
        ...
        parts = list(self.D.keys())
        
        ankers = [list(getattr(part, 'ANKERS').keys()) 
                  for part in list(self.D.values())]
    ...
    def join_parts(self, name1, name2, anker12):
        ...
        for name in [name1, name2]:
            if name in list(self.D.keys()):
                pass
            else: 
                self.add_part(name)

        if anker12[0] in list(getattr(part1, 'ANKERS').keys()):
        ...
        if anker12[1] in list(getattr(part2, 'ANKERS').keys()):
        ...
        if AP1 != AP2:
            self.join_parts(name1, name2, anker12)
        
    def update_joined_parts(self):
        ''' update all joined parts to new locations '''
        for i, k in enumerate(self.joined.keys()):
            k1, k2 = list(self.joined.keys())[i].split(', ')
            V = self.joined[list(self.joined.keys())[i]]
            #print k1, k2, V
    ...
    def get_joined_table(self):
        t = make_table(list(self.joined.keys()), 
               list(self.joined.values()))
        return t
    
    def add_info(self, partname, info):
        if partname in list(self.I.keys()):
            self.I[partname] += '<br>'+info
        else: 
            self.I[partname] = info
    
    def get_info_table(self):
        t = make_table(list(self.I.keys()), 
                       list(self.I.values()))
        return t